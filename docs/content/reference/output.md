...
describes what users get. Pick a format with `-o`, or let govtrack choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
govtrack <command> -o table   # aligned columns for reading
govtrack <command> -o jsonl   # one JSON object per line, for piping
govtrack <command> -o json    # a single JSON array
govtrack <command> -o csv     # spreadsheet friendly
govtrack <command> -o tsv     # tab-separated
govtrack <command> -o url     # just the URL column
govtrack <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
govtrack <command> --fields id,title,url
```
...
```bash
govtrack <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
govtrack <command>            # a table, because this is a terminal
govtrack <command> | wc -l    # JSONL, because this is a pipe
```
