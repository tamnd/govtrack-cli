// Package cli assembles the govtrack command tree from the govtrack
...
// NewApp assembles the kit application from the govtrack domain. The
...
//
// To add a command, declare it in govtrack/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := govtrack.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(govtrack.Domain{}).Register(app)
